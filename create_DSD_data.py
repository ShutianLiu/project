...
def create_DSD_dataset(mode='tr', sr=44100, nfft=2048, nhop=512, 
    ...
    define_path = '/mnt/f/research/data/SiSEC/DSD100/'+str(sr)+'/'+str(nfft)+'/'+str(nhop)
    ...
    if mode is 'tr':
        ...
        mixing_list = '/mnt/f/research/data/SiSEC/DSD100/dev_list_sorted.txt' 
        ...
        mixing_list = '/mnt/f/research/data/SiSEC/DSD100/test_list_sorted.txt' 
        with open(mixing_list) as f:
            for lines in f:
                file_list.append(lines.rstrip('\n'))
    ...
    for i in xrange(num_data):
        ...
        if i == 0:               
            ...
            infeat_set = new_set.create_dataset('infeat', shape=shape,  
                                             dtype=np.float32)
        
            shape = (num_data, T, F)
        
            mix_spec_set = new_set.create_dataset('mix_spec', shape=shape,
                                         dtype=np.float32)
            
            shape = (num_data, T, F, 4)
            
            hard_mask_set = new_set.create_dataset('hard_mask', shape=shape,
                                         dtype=np.float32)
            
            target_mask_set = new_set.create_dataset('target_mask', shape=shape,
                                         dtype=np.float32)
        ...
        print('T and F:')
        print(T,F)    
        infeat_set[i,:T,:] = infeat[:T,:]
        mix_spec_set[i,:T,:] = mix_spec[:T,:]
        hard_mask_set[i,:T] = hard_mask[:T]
        target_mask_set[i,:T] = soft_mask[:T]
